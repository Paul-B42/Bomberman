...
void Init_Joueurs(struct player* Joueurs[NB_j]){
    ...
    Joueurs[2]->x = 0; Joueurs[2]->y = 0; Joueurs[2]->etat = false;
    Joueurs[3]->x = 0; Joueurs[3]->y = 0; Joueurs[3]->etat = false;
}
...
int main(int argc, char** argv) {
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&event) && home == 0) {
            if (event.type == SDL_QUIT) {
                ...
                switch (event.key.keysym.sym) {
                    ...
                    // Joueur 2 
                    case SDLK_UP: 
                        if (Joueurs[1]->frame == 0){
                            move(matrix, 1, 'S', Joueurs);
                        }
                        break;
                    case SDLK_DOWN: 
                        if (Joueurs[1]->frame == 0){
                            move(matrix, 1, 'N', Joueurs);
                        }
                        break;
                    case SDLK_LEFT:
                        if (Joueurs[1]->frame == 0){
                            move(matrix, 1, 'O', Joueurs);
                        }
                        break;
                    case SDLK_RIGHT:
                        if (Joueurs[1]->frame == 0){
                            move(matrix, 1, 'E', Joueurs);
                        }
                        break;

                    case SDLK_u : 
                        if (Joueurs[1]->inventory[0] > 0 && matrix[Joueurs[1]->y][Joueurs[1]->x] == 0){
                            poser_item(&map, Joueurs[1]->x, Joueurs[1]->y, 2);
                            explose[Joueurs[1]->y][Joueurs[1]->x] = SDL_GetTicks()-end;
                            Joueurs[1]->inventory[0] --;
                        }
                        break;
                    case SDLK_i : 
                        if (Joueurs[1]->inventory[1] > 0 && matrix[Joueurs[1]->y][Joueurs[1]->x] == 0){
                            poser_item(&map, Joueurs[1]->x, Joueurs[1]->y, 3);
                            explose[Joueurs[1]->y][Joueurs[1]->x] = SDL_GetTicks()-end;
                            Joueurs[1]->inventory[1] --;
                        }
                        break;
                    case SDLK_o : 
                        if (Joueurs[1]->inventory[2] > 0 && matrix[Joueurs[1]->y][Joueurs[1]->x] == 0){
                            poser_item(&map, Joueurs[1]->x, Joueurs[1]->y, 4);
                            explose[Joueurs[1]->y][Joueurs[1]->x] = SDL_GetTicks()-end;
                            Joueurs[1]->inventory[2] --;
                        }
                        break;
                    case SDLK_p: 
                        if (Joueurs[1]->inventory[3] > 0 && matrix[Joueurs[1]->y][Joueurs[1]->x] == 0){
                            poser_item(&map, Joueurs[1]->x, Joueurs[1]->y, 5);
                            explose[Joueurs[1]->y][Joueurs[1]->x] = SDL_GetTicks()-end;
                            Joueurs[1]->inventory[3] --;
                        }
                        break;
                }
            }
        ...
        sprintf(text, "%d", Joueurs[1]->vie);
        write(text, font, color, renderer, WINDOW_WIDTH - TILE_SIZE, 0);

        // Affichage du nom du joueur
        sprintf(text, "%s // %s", Joueurs[ID]->nom, Joueurs[1]->nom);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sprintf(text, "%d", Joueurs[1]->inventory[i]);
            write(text, font, color, renderer, WINDOW_WIDTH - TILE_SIZE, (i+2)*TILE_SIZE);
        }
